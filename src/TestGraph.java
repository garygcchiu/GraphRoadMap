...
public class TestGraph {
	...
	public static void main(String[] args) {
		...
		/*
		 * Create a graph with 9 nodes and 11 edges. Query the graph and check
		 * that all edges are stored correctly
		 */
		...
		int M[][] = { { 0, 2, 0, 2, 0, 0, 0, 0, 0 }, { 2, 0, 2, 0, 2, 0, 0, 0, 0 }, { 0, 2, 0, 0, 0, 1, 0, 0, 0 },
				{ 2, 0, 0, 0, 2, 0, 1, 0, 0 }, { 0, 2, 0, 2, 0, 2, 0, 2, 0 }, { 0, 0, 1, 0, 2, 0, 0, 0, 2 },
				{ 0, 0, 0, 1, 0, 0, 0, 2, 0 }, { 0, 0, 0, 0, 2, 0, 2, 0, 2 }, { 0, 0, 0, 0, 0, 2, 0, 2, 0 } };
		...
		try {
			...
			for (i = 0; i < 9; ++i)
				for (j = 0; j < i; ++j)
					if (M[i][j] == 1)
						G.insertEdge(V[i], V[j], -1);
					else if (M[i][j] == 2)
						G.insertEdge(V[i], V[j], 0);
			...
			for (i = 0; i < numNodes; ++i) {
				...
				while (neighbours.hasNext()) {
					...
					if (M[node1][node2] == 0) {
						...
						System.out.println("    Error: There should not be a toll road between");
						System.out.println("           nodes " + node1 + " and " + node2);
						failed = true;
					} else if (((uv.getType()) == 0) && (M[node1][node2] != 2)) {
						System.out.println("    Error:There should not be a free road between");
						System.out.println("          nodes " + node1 + " and " + node2);
						failed = true;
					}
				}
				if (degree != NodeDegree[i]) {
					System.out.println(
							"    Error:The degree of node " + i + " should be " + NodeDegree[i] + ", not " + degree);
					failed = true;
				}
			}
